...
var lines = File.ReadAllText(@"../../../../../2024/Exo24/input.txt");
...
static int Evaluate(string label, Circuit circuit, Dictionary<string, int> inputs)
{
...
static IEnumerable<string> Fix(Circuit circuit)
{
...
static IEnumerable<string> SwapAndFix(Circuit circuit, string out1, string out2)
{
    (circuit[out1], circuit[out2]) = (circuit[out2], circuit[out1]);
    return Fix(circuit).Concat([out1, out2]);
}

static string Output(Circuit circuit, string x, string kind, string y) =>
...
static (Dictionary<string, int> inputs, Circuit circuit) Parse(string input)
{
...
record Gate(string in1, string kind, string in2);